...
6. **No server-side case store.** `case_id` is generated when absent and
   echoed back; the service does not persist cases. Persistence is the
   integrator's responsibility.

7. **Verification does not establish authenticity or ownership**, and
   `decision` is not a credit decision.
